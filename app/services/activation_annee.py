...
Responsabilités :
  ...
  2. Exécuter l'activation sous transaction globale atomique (all-or-nothing) :
     ...
     - Resynchronisation de Eleve.classe_id à partir des inscriptions de la nouvelle année active
     - Déconnexion (classe_id = NULL) des élèves sortis, transférés, diplômés ou historiques sans inscription cible
     - Commit unique
  3. Mettre à jour session["annee_consultee"] pour l'école concernée.
  4. Idempotence : si l'année est déjà active, ne rien re-muter.
...
def preparer_activation_annee(ecole_id, annee_id):
    """
    Vérifie l'éligibilité d'une année planifiée à l'activation et
    ...
    if cible.statut == "active":
        return None, "Cette année scolaire est déjà active."
...
def resynchroniser_classes_eleves_annee_active(ecole_id, annee_active_id):
    ...
    Retourne : dict avec (total, synchronises, mis_a_null)
    """
    eleves_ecole = Eleve.query.filter_by(ecole_id=ecole_id).all()
    inscriptions_annee = {
        insc.eleve_id: insc
...
def activer_annee_scolaire(ecole_id, annee_id, user_id=None):
    """
    Exécute la transaction globale atomique d'activation de l'année scolaire :
      1. Revalide tout côté serveur (idempotence, appartenance école, chronologie, bloquants).
      2. Si déjà active : retourne succès idempotent sans modification.
      3. Clôture l'ancienne année active : statut -> 'archivee'.
      4. Active la cible : statut -> 'active'.
      5. Resynchronise Eleve.classe_id pour tous les élèves de l'établissement.
      6. Commit unique.

    En cas d'erreur : rollback intégral et retour d'un statut d'échec.
    ...
    # 1. Idempotence : si l'année est déjà active, ne rien re-muter
    if cible.statut == "active":
    ...
    # 3. Vérification des années actives existantes
    actives = AnneeScolaire.query.filter_by(ecole_id=ecole_id, statut="active").all()
    if len(actives) > 1:
        msg = "Incohérence détectée : plusieurs années scolaires actives."
        return False, msg, {"succes": False, "error": msg}

    ancienne_active = actives[0] if actives else None

    # 4. Vérification chronologique
    if ancienne_active:
        if cible.date_debut <= ancienne_active.date_debut:
            msg = (
                f"Incohérence chronologique : l'année cible ({cible.nom}) débute le "
    ...
    # 5. Revalidation stricte et dynamique des points bloquants
    etat = get_etat_preparation_annee(ecole_id, cible.id)
    if not etat:
    ...
    if not etat["verification"]["prete_pour_activation"] or len(etat["verification"]["bloquants"]) > 0:
        bloquants_str = "; ".join(etat["verification"]["bloquants"])
        msg = f"Activation refusée, préparation incomplète : {bloquants_str}"
        return False, msg, {"succes": False, "error": msg, "bloquants": etat["verification"]["bloquants"]}

    # 6. Exécution transactionnelle atomique (ALL-OR-NOTHING)
    try:
        # A. Clôture de l'ancienne année active
        if ancienne_active:
            ancienne_active.statut = "archivee"

        # B. Activation de la nouvelle année
        cible.statut = "active"

        # C. Resynchronisation du cache Eleve.classe_id
